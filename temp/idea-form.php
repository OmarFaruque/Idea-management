    ...
    <div v-if="listing">
        ...
        <div :class="style.list_row" v-for="item in ideas">
            ...
            <div  :class="[item.user_vote_status ? style.nvote : style.vote]">
                <!-- <span style="background-image:url(<?php //echo esc_url( plugin_dir_url( IM_Idea::$plugin_file ) . 'assets/img/thumb-up.png' ); ?>)"></span>     -->

                    <span :class="style.vote_count"><i>{{item.vote_count}}</i></span>
                    <span :class="style.votespan" v-if="!item.user_vote_status" v-on:click="voteprocess(item.ID, 'pogetive')" style="background-image:url(<?php echo esc_url( plugin_dir_url( IM_Idea::$plugin_file ) . 'assets/img/thumb-up.png' );  ?>)"></span>
                    <span :class="style.nagativevote" v-on:click="voteprocess(item.ID, 'nagative')" v-if="item.user_vote_status" style="background-image:url(<?php echo esc_url( plugin_dir_url( IM_Idea::$plugin_file ) . 'assets/img/thumb-up.png' );  ?>)"></span>
            </div>
        </div>
        <div :class="style.addnewIdea">
            <button v-on:click="gotoNewIdeaForm()"><?php _e('Add New Idea', 'idea-management'); ?></button>
        </div>
    </div>
